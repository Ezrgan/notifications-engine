...
notifications-engine/
  ...
  PLAN.md                   # current phase only (rewritten each time)
  README.md                 # humans (local run; Compose last)
  docs/                     # STATUS + playbook for the next PLAN.md
                                # NOT notifications_engine.egg-info/ (setuptools junk)
  pyproject.toml
  alembic.ini
  docker-compose.yml
  Dockerfile
...
1. **Read** this file, [`PLAN.md`](PLAN.md), [`docs/STATUS.md`](docs/STATUS.md), and the current tree. `PLAN.md` describes **only the current phase**. To write the next one, follow [`docs/HOW_TO_WRITE_THE_NEXT_PLAN.md`](docs/HOW_TO_WRITE_THE_NEXT_PLAN.md). Implement the current phase only. Do not invent later features from §10.1.
...
### 10.3 README vs this file vs EXPLANATIONS.md

- `AGENTS.md` — agents, architecture law, teaching protocol.
- `README.md` — humans: purpose, mermaid diagram, **venv / Postgres / Redis / uvicorn+celery**, curl/Postman, pytest. Compose instructions only after the Docker phase.
- `PLAN.md` — **current phase only**; rewritten when the phase closes. How to write the next one: [`docs/HOW_TO_WRITE_THE_NEXT_PLAN.md`](docs/HOW_TO_WRITE_THE_NEXT_PLAN.md).
- `docs/STATUS.md` — what is actually built. Update at the end of each phase.
- `docs/EXPLANATIONS.md` — **EsrgaN's Knowledge & Study Handbook** (local study notes only). Comprehensive deep-dive explanations of backend architecture, patterns, code trace, Redis, Lua, idempotency, and future topics. Agents must never truncate or delete this file; when new complex architecture slices land (Celery, DLQ, Docker), agents should enrich it with pedagogical depth and examples. **Never commit, push, or open in a PR** — it stays on EsrgaN's machine only (listed in `.gitignore`; not on GitHub).
- Never put docs in `*.egg-info/` (generated by install, gitignored).
